...
public class MysqlPageableSqlParser implements PageableSqlParser {
    ...
    public static final MysqlPageableSqlParser INSTANCE = new MysqlPageableSqlParser();
    ...
    public String replaceAsCountSql(String sql, Pageable pageable) {
        String[] splitFromSqls = PATTERN_FROM.split(sql);
        StringBuilder countSql = new StringBuilder("SELECT COUNT(0) AS count");
        for (int i = 1; i < splitFromSqls.length; i++) {
            countSql.append(" FROM ");
            countSql.append(splitFromSqls[i].trim());
            countSql.append(" ");
        }
