 ...
 * under the License.
 */
package com.icepoint.framework.restdoc.template;

import com.icepoint.framework.restdoc.builder.ProjectDocConfigBuilder;
import com.icepoint.framework.restdoc.constants.*;
import com.icepoint.framework.restdoc.handler.SpringMVCRequestHeaderHandler;
import com.icepoint.framework.restdoc.handler.SpringMVCRequestMappingHandler;
import com.icepoint.framework.restdoc.helper.FormDataBuildHelper;
import com.icepoint.framework.restdoc.helper.JsonBuildHelper;
import com.icepoint.framework.restdoc.helper.ParamsBuildHelper;
import com.icepoint.framework.restdoc.model.*;
import com.icepoint.framework.restdoc.model.request.ApiRequestExample;
import com.icepoint.framework.restdoc.model.request.CurlRequest;
import com.icepoint.framework.restdoc.model.request.RequestMapping;
import com.icepoint.framework.restdoc.parser.MapParamParser;
import com.icepoint.framework.restdoc.util.*;
...
public class SpringBootDocBuildTemplate implements IDocBuildTemplate<ApiDoc> {
    ...
    public List<ApiDoc> getApiData(ProjectDocConfigBuilder projectBuilder) {
        ...
        this.configApiReqParams = Stream.of(apiConfig.getRequestHeaders(), apiConfig.getRequestParams())
        ...
        for (JavaClass cls : classes) {
            if (StringUtil.isNotEmpty(apiConfig.getPackageFilters())
                    && !DocUtil.isMatch(apiConfig.getPackageFilters(), cls.getCanonicalName())) {
                continue;
            }
        ...
        if (apiConfig.isSortByTitle()) {
            ...
            return apiDocList.stream()
                    ...
                    .peek(p -> p.setOrder(atomicInteger.getAndAdd(1)))
                    .collect(Collectors.toList());
        }
        return apiDocList;
    }
    ...
    private List<ApiMethodDoc> buildControllerMethod(final JavaClass cls, ApiConfig apiConfig,
        ...
        for (DocJavaMethod docJavaMethod : docJavaMethods) {
            ...
            if (method.isPrivate() || Objects.nonNull(method.getTagByName(DocTags.IGNORE))) {
                continue;
            }
            ...
            final List<ApiReqParam> apiReqParamList = this.configApiReqParams.stream()
                    .filter(param -> filterPath(requestMapping, param))
                    .collect(Collectors.toList());
            ...
            if (this.configApiReqParams != null) {
                ...
                final Map<String, List<ApiReqParam>> reqParamMap = apiReqParamList.stream()
                        .collect(Collectors.groupingBy(ApiReqParam::getParamIn));

                final List<ApiReqParam> headerParamList = reqParamMap.getOrDefault(ApiReqParamInTypeEnum.HEADER.getValue(), Collections
                        .emptyList());
...
//                        .flatMap(Collection::stream).distinct().collect(Collectors.toList()));
                ...
                allApiReqHeaders = apiReqHeaders.stream()
                        .filter(param -> filterPath(requestMapping, param))
                        .collect(Collectors.toList());
            }
//            allApiReqParams.removeIf(apiReqHeader -> filterPath(requestMapping, apiReqHeader));
        ...
        for (JavaParameter parameter : parameterList) {
            ...
            if (JavaClassValidateUtil.isFile(gicTypeName)) {
                ...
                if (!JavaClassValidateUtil.isPrimitive(gicName)
                    ...
                    log.warning("Spring MVC can't support binding Collection on method "
    ...
    private ApiMethodReqParam requestParams(final DocJavaMethod docJavaMethod, ProjectDocConfigBuilder builder,
        ...
        final Map<String, ApiReqParam> pathReqParamMap = collect.getOrDefault(ApiReqParamInTypeEnum.PATH.getValue(), Collections
                .emptyMap());
        final Map<String, ApiReqParam> queryReqParamMap = collect.getOrDefault(ApiReqParamInTypeEnum.QUERY.getValue(), Collections
                .emptyMap());
        ...
        for (JavaParameter parameter : parameterList) {
            ...
            if (JavaClassValidateUtil.isCollection(fullTypeName) || JavaClassValidateUtil.isArray(fullTypeName)) {
                ...
                // 处理@MapParam
                JavaAnnotation mapParam = methodAnnotations.stream()
                        .filter(annotation -> "MapParam".equals(annotation.getType().getName()))
                        .findAny()
                        .orElse(null);
                boolean hasMapParam = mapParam != null;
                ...
                String[] gicNameArr = DocClassUtil.getSimpleGicName(typeName);
                if (hasMapParam) {
                    paramList.addAll(new MapParamParser(mapParam).parse());

                } else if (JavaClassValidateUtil.isPrimitive(gicNameArr[1])) {
                    ApiParam apiParam = ApiParam.of()
                            .setField(paramName)
                            .setType("map")
                            .setId(paramList.size() + 1)
    ...
    private String createMockValue(Map<String, String> paramsComments, String paramName, String typeName,
    ...
    private static void setTornaArrayTags(List<JavaParameter> javaParameters, ApiMethodDoc apiMethodDoc,
