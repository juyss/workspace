...
public class AttachmentController extends BaseController {
    ...
    @PostMapping(value = "upload/feign", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @Log("上传文件")
    public ResponseBean<Attachment> uploadFeign(
            @ApiParam(value = "要上传的文件", required = true) @RequestPart("file") MultipartFile file,
            Integer uploadType) {
        Attachment attachment = new Attachment();
        if (!file.isEmpty()) {
    ...
    public void download(HttpServletRequest request, HttpServletResponse response, @RequestParam("id") String id) {
        try {
            ...
            InputStream stream = UploadInvoker.getInstance().download(attachment);
            if (stream == null) {
     ...
     * @param id       id
     * @author gaokx
     * @date 2020/12/3 23:47
     */
    @GetMapping("/preview")
    @ApiOperation(value = "预览附件", notes = "根据附件ID预览附件")
    @ApiImplicitParam(name = "id", value = "附件id", required = true, dataType = "String")
    public void preview(HttpServletResponse response, @RequestParam("id") String id) throws Exception {
        Attachment attachment = new Attachment();
        attachment.setId(Long.valueOf(id));
        attachment = attachmentService.get(attachment);
        //支持多存储方式预览
