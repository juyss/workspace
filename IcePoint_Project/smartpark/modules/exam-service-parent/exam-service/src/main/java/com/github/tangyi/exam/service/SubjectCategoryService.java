...
public class SubjectCategoryService extends CrudService<SubjectCategoryMapper, SubjectCategory> {
  ...
  @Caching(evict={
      ...
      @CacheEvict(value = "childrenCategory", allEntries = true)})
	 ...
	 * @return List
	 */
    @Cacheable(value = "categoryTree#" + CommonConstant.CACHE_EXPIRE)
   ...
   * 获得子节点list
   * @param id 传入的匹配id
   * @param selfFlag  是否包含当前节点
   * @return
   */
  @Cacheable(value = "childrenCategory#" + CommonConstant.CACHE_EXPIRE, key = "#id")
  public List<SubjectCategoryDto> getChildCategory(Long id, boolean selfFlag) {
    List menus = categoryTree();
    // 排序、组装树形结构
    return TreeUtil.getChildrenList(menus, id, selfFlag);
  }
