...
public class SubjectUtil {
   ...
   */
  public static List<SubjectDto> subjectToDto(List<Subject> subjects, boolean findAnswer) {
    List<SubjectDto> subjectDtoList = new ArrayList<>();
    if (CollectionUtils.isNotEmpty(subjects)) {
      subjectDtoList = subjects.stream().map(subjectChoice -> SubjectUtil.subjectToDto(subjectChoice, findAnswer)).collect(Collectors.toList());
    }
   ...
   * @date 2019/06/16 16:50
   */
  public static SubjectDto subjectToDto(Subject subject, boolean findAnswer) {
    if (subject == null)
      return null;
    ...
    subjectDto.setAnalysis(subject.getAnalysis());
    ...
    if(SubjectTypeEnum.isChoices(subject.getType())){
      subjectDto.setChoicesType(subject.getType());
    }
    subjectDto.setType(subject.getType());
    subjectDto.setOptions(subject.getOptions());
    subjectDto.setCreator(subject.getCreator());
    subjectDto.setModifier(subject.getModifier());
    subjectDto.setModifyDate(subject.getModifyDate());
    // 参考答案
    if (findAnswer) {
