    ...
    public ResponseBean<Boolean> updateRoleMenu(@RequestBody Role role) {
        ...
        if (role.getId() != null) {
            ...
            if (role != null && StringUtils.isNotBlank(menuIds))
                success = roleMenuService.saveRoleMenus(role.getId(), Stream.of(menuIds.split(",")).filter(item -> StringUtils.isNotBlank(item)).map(Long::parseLong).collect(Collectors.toList())) > 0;
        }
