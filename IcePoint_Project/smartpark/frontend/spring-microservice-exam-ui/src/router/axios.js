...
axios.interceptors.request.use(config => {
  ...
  if (store.getters.access_token && whiteList.indexOf(config.url) === -1) {
    ...
    if (authorization === undefined || authorization.indexOf('Basic') === -1) {
      ...
      if(token.length===36) config.headers['Authorization'] = token // 让每个请求携带token
      else config.headers['Authorization'] = 'Bearer ' + token // 让每个请求携带token
    }
  }
  // 增加租户编号请求头
  const tenantCode = config.headers['Tenant-Code']
  if (tenantCode === undefined) {
    console.log('tenant-code',getTenantCode())
    if(getTenantCode() instanceof String) config.headers['Tenant-Code'] = getTenantCode()
    else config.headers['Tenant-Code'] = 'gitee'
  }
  return config
}, error => {
